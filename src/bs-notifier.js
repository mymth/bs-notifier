...
class Notifier {
  ...
  show(options) {
    ...
    const opt = $.extend(true, {}, this.options, options);
    const $alertBox = $(this.alertBox).addClass(`alert-${opt.type}`);
    let icon = '';
    ...
    if (opt.title) {
      const $titleBox = $(`<${opt.titleTag} />`).addClass(opt.titleClass).html(opt.title);
...
$.fn[pluginName] = function (options) {
  ...
  let plugin = $.data(element, pluginName);
